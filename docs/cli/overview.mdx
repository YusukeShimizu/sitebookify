...
- `--toc-engine` and `--render-engine` accept `codex` or `noop` (default: `codex`).
- Codex settings:
  - `SITEBOOKIFY_CODEX_BIN` (default: `codex`)
  - `SITEBOOKIFY_CODEX_MODEL` (optional)
  - `SITEBOOKIFY_CODEX_REASONING_EFFORT` (optional; e.g. `minimal`, `low`, `medium`, `high`, `xhigh`)
...
```sh
sitebookify toc create --manifest manifest.jsonl --out toc.yaml --book-title "My Book" --language 日本語 --tone 丁寧 --engine codex
```
